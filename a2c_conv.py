...
class A2CAgent:
    def __init__(self, observation_size, action_size, learning_rates, conv_args, fc_args, gamma, saved_weights_dir = None):
        # get size of state and action
        self.observation_size = observation_size
        self.action_size = action_size
        self.value_size = 1

        # Extract the parameters for the convolutional layers
        self.n_output_conv = conv_args[0] # Number of output channels of the convolutional layer
        self.kernel_size = conv_args[1] # Kernel size of the convolutional layer
        self.strides= conv_args[2] # The stride of the convolutional layer
        self.conv_activation_function = conv_args[3]
    ...
    def build_actor_nn(self):
        ...
        actor = Sequential()
        # First use a convolutional layer to handle the RGB-image provided by the environment
        actor.add(Conv2D(self.n_output_conv, 
                        (self.kernel_size, self.kernel_size), 
                        activation=self.conv_activation_function, 
                        strides=self.strides, 
                        input_shape=self.observation_size))
        actor.add(Flatten())
        # Use 2 hidden fully connected layers
        actor.add(Dense(self.n_dense_layer_neurons, activation=self.dense_activation_function, kernel_initializer='he_uniform'))
        actor.add(Dense(self.n_dense_layer_neurons, activation=self.dense_activation_function, kernel_initializer='he_uniform'))
    ...
    def build_critic_nn(self):
        ...
        critic = Sequential()
        # First use a convolutional layer to handle the RGB-image provided by the environment
        critic.add(Conv2D(self.n_output_conv, 
                        (self.kernel_size, self.kernel_size), 
                        activation=self.conv_activation_function, 
                        strides=self.strides, 
                        input_shape=self.observation_size))
        critic.add(Flatten())
        # Use 2 hidden fully connected layers
        critic.add(Dense(self.n_dense_layer_neurons, activation=self.dense_activation_function, kernel_initializer='he_uniform'))
        critic.add(Dense(self.n_dense_layer_neurons, activation=self.dense_activation_function, kernel_initializer='he_uniform'))
    ...
    def load_weights(self):
        ...
        self.critic.load_weights(f"./model/a2c_critic.h5")
        self.actor.load_weights(f"./model/a2c_actor.h5")