...
RENDER = False # Whether or not to save a render of every timestep into the images/ directory
RENDER_DIR = "images/apple_learning/"
SAVE_WEIGHTS = False
LOAD_WEIGHTS = False

## Environment related parameters ##
USE_RANDOM_SPAWN_AGENTS = False # Whether or not to spawn the agent in random positions in every episode
USE_RANDOM_SPAWN_APPLES = False # Whether or not to spawn the apple in a random position in every episode
...
## Environment setup ##
env = AppleLearningEnv(USE_RANDOM_SPAWN_AGENTS, USE_RANDOM_SPAWN_APPLES, num_agents=n_agents) # Create the environment
...
for episode in range(N_EPISODES):
    ...
    for step in range(N_STEPS):
        # If this episode is done, break out of the loop, no need to finish the remaining steps
        if episode_done:
            break
        ...
        for agentKey, agentObject in A2C_agents.items():
            ...
            # If the agent was able to eat the apple indicate that this episode is done
            if (reward==1):
                episode_done = True
            ...
            next_observations = tf.concat([x for x in queue], 0)
            next_observations = np.reshape(next_observations, [1, 1, 15, 15, 3])

            # Update the A2C agent based on the previous and next observations, the chosen action and the reward
            agentObject.train_model(prev_observations, action, reward, next_observations, done)

    # Save the length of this episode
    episode_length[episode] = step+4
    ...
    if (episode % 10) == 0:
        ...
        df = pd.DataFrame(d)
        df.to_csv('results/apple_learning.csv', index=False)
